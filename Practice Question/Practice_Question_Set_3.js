...
const formatArray = (items) =>
  `The array has ${items.length} items, and the fisrt item is "${
    items[0]
...
const formatProduct = (product) => {
  if (product?.inStock) {
    return `${product?.name} costs is INR ${product.price} and is in stock`;
  } else
    return `${product?.name} costs is INR ${product.price} and is not in stock`;
};
...
const findPerson = (item, name) =>
  item.map((val) => (val.name === name ? val : null)).filter(Boolean);

//   item.map((val) => val.name === name).includes(true) ? item : null;
//   item.filter((val) => val.name === name);
//    [ { name: 'Akhil', age: 25 } ]
console.log(
  findPerson(
    [
...
const pickFirstAndSecond = (item) => {
  let [fisrt, second] = item;
  return { fisrt, second };
};
...
const printFirstCharacter = (rest) => rest[0];

console.log(printFirstCharacter([1, 2, 5, 9, 7]));
...
let printSecondCharacter = (arr) => arr[1] * 20;
...
const sumOfEvenIndices = (num) => num.map((val) => (val += val));
...
// 12. 12. Write an ES6 function to return the first element which is a multiple of 5 in the given array.
const printMultipleOfFive = (array) =>
  array.map((val) => (val % 5 == 0 ? val : null)).filter(Boolean)?.[0];
...
const printData = ({ name, country }) => `${name} live in ${country}`;
...
const printProductDetails = ({ name, specification }) =>
  `${name} which is of ${specification.size}, has a capacity of ${specification.capacity}.`;
